...
/*   Updated: 2025/05/03 06:39:27 by dderny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector	vec_insert(const t_vector veca, const t_vector vecb, size_t index)
{
	...
	if (new.data)
	{
		ft_memcpy(new.data, veca.data, index * veca.type_size);
		ft_memcpy(new.data + index * veca.type_size,
			vecb.data, vecb.actual_size * vecb.type_size);
		ft_memcpy(new.data + (index + vecb.actual_size) * veca.type_size,
			veca.data + index * veca.type_size,
			(veca.actual_size - index) * veca.type_size);
	}
	free(veca.data);
	free(vecb.data);
	return (new);
